...
function createUser() {
    ...
    let request = new Request('/admin/users', {
        method: 'POST',
        headers: headers,
...
async function updateUser(userId) {
    ...
    let headers = new Headers();
    headers.append('Content-Type', 'application/json; charset=utf-8');
    let request = new Request("admin/users/" + userId, {
        method: 'PATCH',
        headers: headers,
        body: JSON.stringify(user),
    });
    ...
    await fetch(request).then((response) => {
        response.json().then((userReturned) => {
            ...
            $("#email" + userReturned.id).text(userReturned.username);
...
function getRoles() {
    fetch("/admin/roles").then((response) => {
        if (response.ok) {
